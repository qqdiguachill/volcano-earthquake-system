...
def main():
    ...
    for volcano_id, level_name, name, url in volcanoes:
        ...
        # 存入總表
        cursor.execute("""
        INSERT OR REPLACE INTO volcano_list (id, level_name, volcano_name, url, level, latitude, longitude, elevation)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (volcano_id, level_name, name, url, level, lat, lon, elev))
        conn.commit()
        ...
        for act in activities:
            cursor.execute("""
            INSERT INTO volcano_activity (volcano_id, published, phenomenon, link)
            VALUES (?, ?, ?, ?)
            """, (volcano_id, act["published"], act["phenomenon"], act["link"]))
        conn.commit()
